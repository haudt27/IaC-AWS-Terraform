...
#====================================
# install nginx to run webserver
...
#template to launch instance in auto scaling group
resource "aws_launch_template" "app-template" {
  ...
  user_data = base64encode(data.template_cloudinit_config.config.rendered) #use userdata above
}
...
#create alb target group for webserver
